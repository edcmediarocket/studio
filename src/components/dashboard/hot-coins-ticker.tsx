...
export function HotCoinsTicker() {
  ...
  const fetchTrendingCoins = async () => {
    // setError(null); // Don't reset error immediately on refetch to avoid flicker if it fails again
    try {
      ...
      if (data.coins && data.coins.length > 0) {
        setTrendingCoins(data.coins.map(c => c.item).slice(0, 7)); // Take top 7 coins
        setError(null); // Clear error on successful fetch
      } else {
        setTrendingCoins([]);
        // setError("No trending coins found at the moment.");
      }
    } catch (err) {
      ...
      if (err instanceof Error) {
       ...
       // Keep existing coins if fetch fails, to avoid empty ticker unless it's the first load
  ...
  if (error && trendingCoins.length === 0) {
    return (
      <div className="h-16 flex items-center justify-center bg-destructive/20 border-y border-destructive text-destructive-foreground px-4">
  ...
  if (trendingCoins.length === 0 && !loading) {
    return (
       <div className="h-16 flex items-center justify-center bg-card border-y border-primary/50 text-muted-foreground">
  ...
  const duplicatedCoins = [...trendingCoins, ...trendingCoins];
          ...
          {duplicatedCoins.map((coin, index) => (
            <Link href={`/coin/${coin.id}`} key={`${coin.id}-${index}`} className="flex items-center mx-4 p-2 rounded-lg hover:bg-primary/10 transition-colors shrink-0 w-64">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-semibold text-foreground truncate">{coin.name} ({coin.symbol.toUpperCase()})</span>
                <span className="text-xs text-muted-foreground">Rank: #{coin.market_cap_rank || 'N/A'}</span>
              </div>
               <Flame className="ml-auto h-5 w-5 text-neon opacity-70" />
            </Link>
          ))}
        </div>
      </div>
