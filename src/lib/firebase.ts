...
const firebaseConfig: FirebaseOptions = {
  apiKey: "AIzaSyD1Z4ELRKb-9Ic2IgVy99Q5NUt43kI879o", // Re-inserting the last known good key
  ...
  try {
    ...
    if (permission === 'granted') {
      ...
      const token = await getToken(messagingInstance, {
        vapidKey: 'BDsu4ZwCLkC-kLMQXZap-cyDs1Uq-5oDDKmH6ow0gWuagzVHmgLO-ta8vdXsjylg3yMyUjFDS6NE4nsH7X_DItg', // Your VAPID key
      });
      console.log('FCM Token retrieved in requestPermission:', token);
      // Placeholder for sending token to backend:
      // await sendTokenToBackend(auth.currentUser?.uid, token);
      return token;
    } else {
