...
const GetSimulatedSignalPerformanceOutputSchema = z.object({
  ...
  simulatedProfitFactor: z.number().describe('Simulated profit factor (e.g., 1.5 means $1.50 conceptual profit for every $1 conceptual loss from past signals, or 0.8 if it lost money). This is a conceptual estimation.'),
  keyStrengths: z.array(z.string()).describe('List of 2-3 simulated key strengths of the AI signal strategy for this coin (e.g., "Good at identifying short-term breakouts", "Effective during high volatility periods").'),
  keyWeaknesses: z.array(z.string()).describe('List of 2-3 simulated key weaknesses or limitations of the AI signal strategy for this coin (e.g., "Less accurate during low-volume consolidation", "Prone to false signals on purely news-driven pumps").'),
...
Your output MUST adhere to the JSON schema. Provide all fields, including:
- 'coinName': This should be "{{coinName}}".
...
- 'simulatedProfitFactor': A plausible profit factor (e.g., between 0.7 and 3.0). This is total conceptual gains divided by total conceptual losses.
...
- 'lastSimulatedBacktestDate': Provide a recent-sounding date (e.g., "May 15, 2025").
- 'disclaimer': Ensure the standard disclaimer is included.

Focus on generating plausible and insightful-sounding metrics and descriptions for {{coinName}}. The goal is to give a *sense* of potential past performance, not an actual backtest.
`,
});
...
const getSimulatedSignalPerformanceFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (output) {
      ...
      // coinName and lastSimulatedBacktestDate will be handled by the wrapper function for final accuracy
    }
    return output!;
  }
);
