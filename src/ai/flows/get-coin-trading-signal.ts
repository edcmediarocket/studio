...
const GetCoinTradingSignalOutputSchema = z.object({
  ...
  tradingSessionInsights: z.string().optional().describe("AI-generated insights on how current or upcoming global trading sessions (e.g., U.S. open, Asian session, European session) might influence the trading strategy for this coin. For each of these three sessions, discuss characteristic patterns, optimal periods for certain strategies, or key timing considerations for entries or exits, based on session-specific liquidity and volatility patterns. This should be tailored to the user's selected tradingStyle."),
...
10. **'tradingSessionInsights' (Optional but Encouraged)**: Provide detailed insights on how current or upcoming major global trading sessions (U.S. open, Asian session, European session) might influence the trading strategy for "{{coinName}}". **For each of these three sessions, discuss characteristic patterns, optimal periods for certain strategies, or key timing considerations for entries or exits,** based on session-specific liquidity and volatility patterns. Tailor these insights to the user's selected 'tradingStyle'.
    *   **For example, for the U.S. session:** "Consider monitoring for increased volatility around the U.S. market open (9:30 AM EST) for potential breakout opportunities if following a momentum style. Key support/resistance levels often tested during the first 1-2 hours."
    *   **Similarly, provide specific examples or characteristic patterns for the European session:** (e.g., "The European session (approx 3 AM - 12 PM EST) often sees initial reactions to overnight news from Asia and can set the tone before U.S. markets open. Swing traders might look for consolidation breakouts or trend continuations during mid-European hours (e.g., 7-10 AM EST), while scalpers could focus on liquidity around major market overlaps like London/Frankfurt open.")
    *   **And for the Asian session:** (e.g., "Liquidity may be thinner during late Asian hours (e.g., after 2 AM EST / during Tokyo lunch break), so conservative traders should be cautious with large orders or widen stops. Early Asian session (e.g., around 8 PM - 10 PM EST previous day) can sometimes follow U.S. closing momentum, offering opportunities for trend-following styles if volume supports. Watch for specific news releases from Asian markets that might impact local sentiment.")
