...
const GetCustomizedCoinTradingSignalOutputSchema = z.object({
  ...
  reasoning: z.string().describe('Concise summary explaining how the selected timeframe and risk profile influenced the recommendation.'),
  confidenceScore: z.number().min(0).max(100).int().describe('A numerical confidence score (0-100) for the generated signal.'),
  detailedAnalysis: z.string().describe('In-depth analysis supporting the signal, considering the specified timeframe and risk. This should cover relevant technical, fundamental, and sentiment factors if applicable for a meme coin.'),
  ...
  strategyNotes: z.string().describe('Specific notes or advice on how to approach trading this signal, considering the user\'s timeframe and risk tolerance (e.g., "For a high-risk, 1H timeframe, consider quick scalping." or "For a low-risk, 1W timeframe, look for broader trend confirmations.").'),
...
- 'reasoning': Briefly explain how the {{timeframe}} and {{riskProfile}} led to this recommendation. For example, a 'High' risk profile on a '1H' timeframe might lead to a more aggressive 'Buy' on a small dip, whereas a 'Low' risk profile on a '1D' timeframe would require stronger confirmation.
- 'confidenceScore': Provide an integer score from 0 to 100.
- 'detailedAnalysis': Elaborate on the factors (technical setup for the given timeframe, current sentiment, any relevant news, tokenomics impact on short/long term) that support your signal. Specifically address how the {{timeframe}} (e.g., short-term indicators for 1H vs. longer trends for 1W) and {{riskProfile}} (e.g., tighter stop-losses for high risk, wider for low risk) shape this analysis.
- 'priceTargets': Provide specific numerical currency values for entry, stop-loss, and take-profit levels. These targets MUST reflect the selected {{timeframe}} and {{riskProfile}}. Shorter timeframes and higher risk might have tighter targets.
- 'strategyNotes': Offer actionable advice related to executing a trade based on this signal, tailored to the user's inputs. For instance, how might position sizing differ, or what specific chart patterns to watch for confirmation on the given timeframe.
...
Prioritize providing realistic and actionable insights for a speculative asset like a meme coin, adjusted for the user's specific request.
`,
});
